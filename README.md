...
| **Absolute Difference** | The core detection step. We use `cv2.absdiff(test, template)` to generate a difference map. This is crucial because a defect might be a **missing** copper line (darker than background) or an **extra** solder blob (lighter than background). The absolute difference ensures both types show up as high-intensity pixels. |
| **Otsu Thresholding** | The difference map contains varying intensity pixels. Instead of manually tuning a threshold value, **Otsu's method** is used to automatically find the optimal threshold that best separates the foreground (defects) from the background noise. This yields a clean, binary mask (`cv2.THRESH_OTSU`). |
...
We will be implementing, training, and evaluating the EfficientNet-B4 model using the data/processed dataset generated in Milestone 1.
